...
export default function NewProjectPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [companiesRes, writersRes, tagsRes] = await Promise.all([
        fetch('/api/companies'),
        fetch('/api/contacts?type=writer'),
      ...
      if (writersRes.ok) {
        const writerData = await writersRes.json()
  ...
  const selectedWriters = writers.filter((writer) => selectedWriterIds.includes(writer.id))
  const filteredWriters = writers
    .filter((writer) => writer.name.toLowerCase().includes(writerSearch.trim().toLowerCase()))
    .slice(0, 12)
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-xl shadow-[0_1px_3px_rgba(16,24,40,0.06)] p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">Writers</label>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {selectedWriters.map((writer) => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full text-xs border bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF] hover:bg-[#DBEAFE]"
                    >
                      {writer.name} x
                    </button>
                  ))}
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:border-transparent"
                placeholder="Search existing writers"
              />
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border text-left text-sm transition-colors ${
                        selected
                          ...
                          : 'bg-white border-[#E4E7EC] text-slate-700 hover:bg-[#F8F9FB]'
                      }`}
                    >
                      {writer.name}
                    </button>
              ...
              <p className="mt-2 text-xs text-slate-500">Attach every credited writer. New writers can still be created from Contacts or from the project team page.</p>
            </div>
            <div className="md:col-span-2">
