...
import { findOrCreateContact } from '@/lib/mcp-intake'

export async function POST(request: NextRequest) {
  const actor = await requireMcpAuth(request, 'intake:write')
  if (isMcpAuthResponse(actor)) return actor
  try {
    const result = await findOrCreateContact(actor, await request.json())
    return mcpOk(result, { actor: actor.id, action: result.created ? 'created' : 'found' })
  } catch (error) {
    return mcpError('intake_contact_failed', error instanceof Error ? error.message : 'Failed to find or create contact')
  }
}
