...
function testPrimaryNavRoutes() {
  const routes = [
    '/', '/contacts', '/contacts/new', '/projects', '/projects/new', '/meetings', '/meetings/new', '/activity', '/materials', '/agreements', '/coverage', '/coverage/new', '/settings', '/buyers', '/whiteboard', '/ip',
  ]
...
function testDashboardRedesignContracts() {
  ...
  const dashboardPage = fs.readFileSync(path.join(__dirname, '../src/app/page.tsx'), 'utf8')
  const unreadWidget = fs.readFileSync(path.join(__dirname, '../src/components/DashboardUnreadScripts.tsx'), 'utf8')

  const materialTypesBlock = helperModule.slice(helperModule.indexOf('SCRIPT_MATERIAL_TYPES'), helperModule.indexOf('export type AgeTone'))
  assert(materialTypesBlock.includes('SCRIPT_MATERIAL_TYPES'), 'Dashboard helpers should define full script material types')
  assert(materialTypesBlock.includes('PITCH_DECK_MATERIAL_TYPES'), 'Dashboard helpers should define pitch deck material types')
  assert(materialTypesBlock.includes("'PILOT_SCRIPT'"), 'Script dashboard material types should include pilot scripts')
  assert(materialTypesBlock.includes("'FEATURE_SCRIPT'"), 'Script dashboard material types should include feature scripts')
  assert(materialTypesBlock.includes("'PITCH_DECK'"), 'Pitch deck dashboard material types should include pitch decks')
  assert(materialTypesBlock.includes("'ONE_PAGER'"), 'Pitch deck dashboard material types should include one pagers')
  assert(materialTypesBlock.includes("'TREATMENT'"), 'Pitch deck dashboard material types should include treatments/outlines')
  assert(materialTypesBlock.includes("'SERIES_BIBLE'"), 'Pitch deck dashboard material types should include series bibles')
  assert(!materialTypesBlock.includes("'OTHER'"), 'Readable dashboard material types must exclude CVs, forms, and other attachments')
  ...
  assert(unreadWidget.includes('emptyLabel'), 'Dashboard should render configurable clean unread empty states')
  assert(dashboardPage.includes('unreadScriptTypeCounts'), 'Dashboard should show unread full script counts by type')
  assert(dashboardPage.includes('unreadPitchDeckTypeCounts'), 'Dashboard should show unread pitch deck counts by type')
  ...
  assert(dashboardPage.includes('Next Steps Dashboard'), 'Dashboard should include project next steps rollup')
  assert(dashboardPage.includes('projectsWithNextSteps'), 'Dashboard should query projects with saved next steps')
  assert(dashboardPage.includes('splitNextSteps'), 'Dashboard should split saved project next actions into individual next steps')
  assert(!dashboardPage.includes('Relationship risk'), 'Dashboard should not render the old relationship risk widget')
  ...
  assert(unreadWidget.includes('useEffect') && unreadWidget.includes('setRows(initialRows)'), 'Dashboard unread widget should resync rows when sort navigation returns a fresh unread list')
...
function testUsabilityFiltersAndSearch() {
  ...
  assert(projectsPage.includes('name="sort"'), 'Project search should preserve the selected sort')
  assert(projectsPage.includes('contacts: { some: { contact: { name:'), 'Project search should include writer/contact names')
  assert(projectsPage.includes('type: true,') && projectsPage.includes('createdAt: true,'), 'Projects page should load material types and upload timestamps')
  assert(projectsPage.includes('a.materials[0]?.createdAt'), 'Projects page date sorting should use latest material upload timestamps')
  assert(projectsPage.includes('aLatestUpload === null && bLatestUpload !== null'), 'Projects without materials should sort after projects with uploads')
  assert(projectsPage.includes('Latest upload') && projectsPage.includes('Email received'), 'Projects page should distinguish upload recency from original email received date')
  assert(projectsPage.includes('materialTypeLabels') && projectsPage.includes('materialCounts'), 'Projects page should summarize available material types and counts')
  assert(projectsPage.includes('projectsHref(params'), 'Project filters and sorts should preserve active query parameters')
  assert(projectsPage.includes('effectiveProjectVerdict'), 'Projects page should compute a read-only effective verdict')
  assert(projectsPage.includes('if (project.verdict) return project.verdict'), 'Explicit project verdict should take precedence over coverage verdicts')
  assert(projectsPage.includes('project.materials.flatMap((material) => material.coverages)'), 'Effective verdict should include coverage linked through project materials')
  assert(projectsPage.includes('project.effectiveVerdict === params.verdict?.toUpperCase()'), 'Verdict tabs should filter by effective verdict')
  assert(projectsPage.includes('effectiveVerdict: effectiveProjectVerdict(project)'), 'Displayed project verdicts should use the effective verdict')
  assert(projectsPage.includes('href={`/projects/${project.id}`} className="font-medium text-green-700'), 'Coverage available should open project detail where direct and material-linked coverage are both visible')
